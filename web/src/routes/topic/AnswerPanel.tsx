    ...
    return (
      <Dialog.Root open={question !== null} onOpenChange={(open) => !open && onClose()}>
        <Dialog.Portal>
          ...
          >
            {question && (
              <PanelBody question={question} onClose={onClose} title={<Dialog.Title className={TITLE}>Mark scheme</Dialog.Title>} />
            )}
          </Dialog.Content>
        </Dialog.Portal>
      </Dialog.Root>
          ...
          <div className="space-y-4">
            ...
            {a.mark_scheme_text && (
              <details className="rounded-md border border-rule">
                ...
                <pre className="m-0 whitespace-pre-wrap break-words border-t border-rule px-3 py-3 font-mono text-[13px] leading-relaxed text-ink">
                  {a.mark_scheme_text}
                </pre>
              </details>
            )}
